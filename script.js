...
function attachProjectNavigation(projectItem, projectInfo) {
  ...
  projectItem.addEventListener("click", (event) => {
    ...
    const projectRail = projectItem.closest(".project-grid");

    if (projectRail?.dataset.dragScrollSuppressClick === "true") {
      return;
    }
...
const projectRail = document.querySelector(".projects .project-grid");

function setupProjectRailDragScroll(rail) {
  if (!rail) {
    return;
  }
  ...
  function queueScrollUpdate(nextScrollLeft) {
    ...
    dragFrame = window.requestAnimationFrame(applyPendingScroll);
  }

  function clearSuppressedClick() {
    rail.dataset.dragScrollSuppressClick = "false";
  }

  function suppressNextClick() {
    rail.dataset.dragScrollSuppressClick = "true";
    window.clearTimeout(suppressClickTimer);
    suppressClickTimer = window.setTimeout(clearSuppressedClick, 140);
  }

  function canDragRail() {
    return (
  ...
  function syncRailState() {
    rail.classList.toggle("is-drag-scroll-enabled", canDragRail());
  }
  ...
  rail.addEventListener("pointerdown", (event) => {
    ...
    if ((event.pointerType && event.pointerType !== "mouse" && event.pointerType !== "pen") || event.button !== 0) {
      return;
    }
    ...
    pendingScrollLeft = rail.scrollLeft;
    hasDragged = false;
  ...
  rail.addEventListener("pointermove", (event) => {
    ...
    queueScrollUpdate(startScrollLeft - deltaX);
  ...
  rail.addEventListener(
    "click",
    (event) => {
      if (rail.dataset.dragScrollSuppressClick !== "true") {
        return;
      }
  ...
  mobileRailQuery.addEventListener?.("change", syncRailState);
}
